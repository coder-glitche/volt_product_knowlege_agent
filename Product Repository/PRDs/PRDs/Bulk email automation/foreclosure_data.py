...
def main():
    ...
    with SSHTunnelForwarder(
        ...
        foreclosure_email= pd.read_sql_query(""" 
                        select fr.collection_id,ba.account_holderpan,ba.account_holder_name,fr.request_id, 
                                            cm.lender_loan_account_number, 
                                             cm.lender_credit_id,fr.status,fr.created_on, fr.last_updated_on , 
                                             fr.lender_id, fr.credit_id, fr.amount_due
                            from
							(SELECT account_id, associated_collection_id as collection_id,
									...
									WHERE created_on + INTERVAL '5 hours 30 minutes' < (CURRENT_DATE + INTERVAL '12 hours 30 minutes')
									  AND created_on + INTERVAL '5 hours 30 minutes' > (CURRENT_DATE - 1 + INTERVAL '12 hours 30 minutes')) fr
                            left join
    ...
    with SSHTunnelForwarder(
        ...
        query = f"""
                SELECT collection_id, actual_amount_collected, transaction_id, 
            COALESCE(
    ...
    foreclosure_collections = foreclosure_collections.rename(columns={
                                    'credit_id': 'Credit ID',
                                    'lender_id': 'Lender partner ID',
                                    'lender_loan_account_number': 'Loan Account Number',
                                    'lender_credit_id': 'Loan Contract Number',
                                    'account_holder_name': 'Customer Name',
                                    'request_id': 'VOLT Request ID',
                                    'created_on': 'Request created On',
                                    'account_holderpan': 'Customer PAN',
                                    'amount_due': 'Net Payable',
    ...
    foreclosure_collections= foreclosure_collections[['Credit ID', 
                                                      'Lender partner ID', 
                                                      'Loan Account Number', 
    ...
    foreclosure_collections_tata = foreclosure_collections[foreclosure_collections['Lender partner ID'] == 'Tata']
    foreclosure_collections_Bajaj = foreclosure_collections[foreclosure_collections['Lender partner ID'] == 'Bajaj']
