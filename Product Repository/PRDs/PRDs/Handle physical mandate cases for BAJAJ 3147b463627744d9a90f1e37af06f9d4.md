# Handle physical mandate cases for BAJAJ

: Ranjan kumar Singh
Created time: September 2, 2024 2:10 PM
Status: In progress
...
- Now, when customer are required to do the physical mandate, ops need to use two admin action to skip e-mandate and add document step to upload physical mandate form.
- Ask: ops ask is to provide one admin action which enable ops to skip the mandate and add document step using single admin action.
...
- Provide toggle button to add document step
    - When toggle is “ON” which means e-mandate will be skipped and document step will be added to upload the physical mandate form
    - By default toggle will be set as “ON”

Note: 

- When “Approve mandate” admin action is used with “Add document step” toggle “ON” then add document step else do not add document step.
- Above admin action should not impact the AS/ES document upload flow.
