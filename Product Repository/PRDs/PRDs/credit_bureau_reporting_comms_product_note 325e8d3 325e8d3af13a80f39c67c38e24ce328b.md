# credit_bureau_reporting_comms_product_note 325e8d3af13a808b82ebe94969cbc741
...
- .As part of regulatory obligations, DSP Finance is required to report borrowers with outstanding interest dues to credit bureaus within a defined reporting window.
    - **Who is facing the problem:**
        ...
        - **Technology / Engineering team (internal):** No existing trigger or job in place to dispatch comms at the point of bureau reporting; all communication today is absent for this event
...
- **Outcome 2 — ≥ 95% of identified defaulters receive at least one channel notification within x hour of reporting job completion**
    - The communication job must complete within x minutes of the reporting event and achieve delivery to ≥ 95% of the target population on at least one channel (SMS or Email)
...
| Job completion SLA | Within x minutes of reporting event | Job timestamp audit |
...
| Retry logic | x retries at y-minute intervals on API failure |
...
- **End-to-end process flow — Bureau Reporting Communication:**
    ...
    - **Step 3 — Comms Job Trigger (System)**
        - Comms job is triggered by the reporting date triger time.
...
- **Error / edge case flows:**
    - **API timeout:** Retry up to x times at y-minute intervals → if all retries fail, raise manual alert to compliance team; compliance team manually confirms communication obligation and triggers fallback
...
| Comms trigger | On bureau reporting day completion | On bureau reporting day completion |
